...
4500 in the 25th iteration, but didn't reach that in the second round of
...
We'll use those settings going forward.
...
py_set_seed(17)
recommendations = campaign$recommend(batch_size = 26L) # IPO does 26 at a time
...
recommendations.1 = campaign$recommend(batch_size = 26L) # Batch size should equal what IPO does
...
Peak picking score increased to 4041. But how do we know when we're done?
...
```{r}
run.iteration = function(campaign, qc.data, batch_size = 26L) {
  ...
  c(recommendations = recs, scores = pps.scores, measurements = measurements)
}
...
This took about 10 minutes to run, and after 3 additional iterations, the
optimization converged with a best PPS of 4137, about the same as IPO.
...
history[[7]]$measurements.ppm[which.max(history[[7]]$scores.PPS)]

cwp.baybe = CentWaveParam(
  ppm = history[[7]]$measurements.ppm[which.max(history[[7]]$scores.PPS)],
  peakwidth = c(
    history[[7]]$measurements.min_peakwidth[which.max(history[[7]]$scores.PPS)],
    history[[7]]$measurements.max_peakwidth[which.max(history[[7]]$scores.PPS)]
  ),
  mzdiff = history[[7]]$measurements.mzdiff[which.max(history[[7]]$scores.PPS)]
)

peaks.baybe = findChromPeaks(qc.data, param = cwp.baybe)

caffeine.mzr = calcPPM(195.0877, 10)
peaks.baybe |> 
  filterMz(c(caffeine.mzr[1], caffeine.mzr[2])) |>
  chromatogram() |> 
  plot()
...
These results look pretty close to what I ended up with by manually tweaking
the parameters.
...
peaks.manual = findChromPeaks(qc.data |> filterRt(c(25, 480)), param = cwp)
calcPPS(peaks.manual)
peaks.manual |> 
  filterMz(c(caffeine.mzr[1], caffeine.mzr[2])) |>
  chromatogram() |> 
  plot()
...
calcPPS(peaks.claude)
```

This looks reasonable, but the PPS score is only 1061, the worst of the
...
ipo.table = map_dfr(seq_along(pp.results), ~ {
  tibble(
    method = 'IPO',
    iteration = .x,
    best.pps = max(pp.results[[.x]]$response[,'PPS'], na.rm = TRUE)
  )
}) |>
  mutate(cumulative.best = cummax(best.pps))
# Last item in list is the result, so remove last row
ipo.table = ipo.table[-nrow(ipo.table), ]

convergence.table = map_dfr(seq_along(history), ~ {
  tibble(
    method = 'BayBE',
    iteration = .x,
    best.pps = max(history[[.x]]$scores.PPS, na.rm = TRUE)
  )
}) |>
  mutate(cumulative.best = cummax(best.pps)) |>
  bind_rows(ipo.table)
...
convergence.table = convergence.table |>
  bind_rows(tibble(
    method = 'Manual',
    iteration = 3,
    best.pps = calcPPS(peaks.manual)['PPS'],
    cumulative.best = best.pps
  ), tibble(
    method = 'LLM',
    iteration = 1,
    best.pps = calcPPS(peaks.claude)['PPS'],
    cumulative.best = best.pps
  ))
convergence.table
ggplot(convergence.table, 
  aes(x = iteration, y = cumulative.best, color = method)) +
  geom_line() +
  geom_point(aes(y = best.pps)) +
  labs(
...
So finally, the order of performance is BayBE > Manual > IPO > LLM.
BayBE was able to find a better optimum than IPO, and when parallelized,
should be faster than IPO. Manual optimization yielded better PPS than IPO,
but required a lot of experience in dealing with this kind of data. Claude
performed the worst out of the four methods, but used a conservative
peak quality metric. Like anything with LLMs, it's dangerous to use its
outputs without checking for yourself first.
